...
def initialise_players(attacking_team,defending_team,attacking_teamname,defending_teamname,params,removed_player):
    """
    ...
    """    
    # get player ids
    attacking_player_ids = np.unique( [ c.split('_')[1] for c in attacking_team.keys() if c[:4] == attacking_teamname ] )
    defending_player_ids = np.unique( [ c.split('_')[1] for c in defending_team.keys() if c[:4] == defending_teamname ] )
    # remove attacking player near the disc
    if removed_player is not None:
        attacking_player_ids = attacking_player_ids[attacking_player_ids != str(removed_player)[-1]]
        # remove the nearest difending player within 3 meters of the attacking player to be removed
        disc_holder_loc = np.array( [ attacking_team[_] for _ in attacking_team.keys() if _[5] == str(removed_player)[-1] and len(_) == 8 ] )
        dist_min = 100
        for i in defending_player_ids:
            defending_player_loc = np.array( [ defending_team[_] for _ in defending_team.keys() if _[5] == i and len(_) == 8 ] )
            dist = np.linalg.norm(disc_holder_loc - defending_player_loc)
            dist_min = min(dist, dist_min)
            if dist == dist_min:
                defending_removed_player = i
        if dist_min <= 3.:
            defending_player_ids = defending_player_ids[defending_player_ids != defending_removed_player]
    else:
        disc_holder_loc = None
    # create list
    attacking_players = []
    for p in attacking_player_ids:
        # create a player object for player_id 'p'
        attacking_player = player(p,attacking_team,attacking_teamname,params)
        if attacking_player.inframe:
            attacking_players.append(attacking_player)
    defending_players = []
    for p in defending_player_ids:
        # create a player object for player_id 'p'
        defending_player = player(p,defending_team,defending_teamname,params)
        if defending_player.inframe:
            defending_players.append(defending_player)
    return attacking_players, defending_players, disc_holder_loc
...
def default_model_params(time_to_control_veto=3):
    ...
    params['reaction_time'] = 0.3 # seconds, time taken for player to react and change trajectory. Roughly determined as vmax/amax
    ...
    params['kappa_def'] =  1.2 # kappa parameter in Spearman 2018 (=1.72 in the paper) that gives the advantage defending players to control ball, I have set to 1 so that home & away players have same ball control probability
    ...
    params['average_ball_speed'] = 10. # average ball travel speed in m/s
    # numerical parameters for model evaluation
    params['int_dt'] = 1/30 # integration timestep (dt)
...
#def remove_players_near_disc(events, )

def generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, removed_players, params, field_dimen = (54.0,20.0,), n_grid_cells_x = 50, offsides=False, remove=False):
    ...
    frame = pass_frame - events.loc[0]['Start Frame']
    ...
    xgrid = np.linspace(0, n_grid_cells_x, 50)*dx - field_dimen[0]/2. + dx/2.
    ygrid = np.linspace(0, n_grid_cells_y, 32)*dy - field_dimen[1]/2. + dy/2.
    ...
    if remove is False:
        attacking_players, defending_players, disc_holder_loc = initialise_players(tracking_home.loc[pass_frame],tracking_away.loc[pass_frame],'Home','Away',params,None)
    else:
        attacking_players, defending_players, disc_holder_loc = initialise_players(tracking_home.loc[pass_frame],tracking_away.loc[pass_frame],'Home','Away',params,removed_players.iloc[int(frame), 0])
    ...
    return PPCFa,xgrid,ygrid,attacking_players,disc_holder_loc
...
def generate_pitch_control_for_tracking(tracking_home, tracking_away, tracking_frame, attacking_team, params, field_dimen = (54.0,20.0,), n_grid_cells_x = 50):
    ...
    # pass_frame = events.loc[event_id]['Start Frame']
    # pass_team = events.loc[event_id].Team
    ball_start_pos = np.array([tracking_home.loc[tracking_frame]['ball_x'],tracking_home.loc[tracking_frame]['ball_y']])
    # break the pitch down into a grid
    n_grid_cells_y = int(n_grid_cells_x*field_dimen[1]/field_dimen[0])
    dx = field_dimen[0]/n_grid_cells_x
    dy = field_dimen[1]/n_grid_cells_y
    ...
    if attacking_team=='Home':
        attacking_players = initialise_players(tracking_home.loc[tracking_frame],'Home',params)
        defending_players = initialise_players(tracking_away.loc[tracking_frame],'Away',params)
    elif attacking_team=='Away':
        defending_players = initialise_players(tracking_home.loc[tracking_frame],'Home',params)
        attacking_players = initialise_players(tracking_away.loc[tracking_frame],'Away',params)
    else:
