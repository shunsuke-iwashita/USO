...
def generate_pitch_control_for_tracking(tracking_home, tracking_away, tracking_frame, attacking_team, params, GK_numbers, field_dimen = (106.,68.,), n_grid_cells_x = 50, offsides=True):
    ...
    if attacking_team=='Home':
        attacking_players = initialise_players(tracking_home.loc[tracking_frame],'Home',params,GK_numbers[0])
        defending_players = initialise_players(tracking_away.loc[tracking_frame],'Away',params,GK_numbers[1])
    elif attacking_team=='Away':
        defending_players = initialise_players(tracking_home.loc[tracking_frame],'Home',params,GK_numbers[0])
        attacking_players = initialise_players(tracking_away.loc[tracking_frame],'Away',params,GK_numbers[1])
    else:
        assert False, "Team in possession must be either home or away"
