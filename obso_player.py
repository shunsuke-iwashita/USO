...
def calc_obso(PPCF, Transition, Score, tracking, attack_direction=0):
    ...
    ball_grid_x = int((tracking['ball_x']+ 52.5) // (54/50))
    ball_grid_y = int((tracking['ball_y']+ 34) //  (20/32))
...
def calc_player_evaluate_match(OBSO, events, tracking_home, tracking_away, args):
    ...
    home_index = list(np.sort(list(set(range(len(events[events['Team']=='Home']))) & set(range(args.start_ev,args.end_ev)))))
    away_index = list(np.sort(list(set(range(len(events[events['Team']=='Away']))) & set(range(args.start_ev,args.end_ev)))))
    ...
    home_event_num = 0
    away_event_num = 0
    frame0 = tracking_home.index[0]
    for num, frame in enumerate(tqdm(events['Start Frame'][args.start_ev:args.end_ev])):
        if events['Team'].iloc[num] == 'Home':
            ...
            for player in home_player_num:
                home_player_pos = [tracking_home[player+'_x'].iloc[frame-frame0], tracking_home[player+'_y'].iloc[frame-frame0]]
            ...
            for player in away_player_num:
                away_player_pos = [tracking_away[player+'_x'].iloc[frame-frame0], tracking_away[player+'_y'].iloc[frame-frame0]]
                if not np.isnan(away_player_pos[0]):
                    try: away_obso[player].iloc[away_event_num-1] = calc_player_evaluate(away_player_pos, OBSO[num])
...
def calc_onball_obso(events, tracking_home, tracking_away, home_obso, away_obso, args):
    ...
    for num, frame in enumerate(tqdm(events['Start Frame'][args.start_ev:args.end_ev])):
        if events['Team'].iloc[num] == 'Home':
            ...
            for name in home_name:
                if np.isnan(tracking_home[name+'_x'].iloc[frame-frame0]):
                    continue
                else:
                    # initialize distance in format dictionary
                    player_pos = np.array([tracking_home[name+'_x'].iloc[frame-frame0], tracking_home[name+'_y'].iloc[frame-frame0]])
                    ball_pos = np.array([tracking_home['ball_x'].iloc[frame-frame0], tracking_home['ball_y'].iloc[frame-frame0]])
            ...
            for name in away_name:
                if np.isnan(tracking_away[name+'_x'].iloc[frame-frame0]):
                    continue
                else:
                    # initialize distance in format dictionary
                    player_pos = np.array([tracking_away[name+'_x'].iloc[frame-frame0], tracking_away[name+'_y'].iloc[frame-frame0]])
                    ball_pos = np.array([tracking_away['ball_x'].iloc[frame-frame0], tracking_away['ball_y'].iloc[frame-frame0]])
                    ball_dis = np.linalg.norm(player_pos-ball_pos)
                    dis_dict[name] = ball_dis
...
def remove_offside_obso(events, tracking_home, tracking_away, home_obso, away_obso, args):
    ...
    for event_id, frame in enumerate(tqdm(events['Start Frame'][args.start_ev:args.end_ev])):
        # for event_id in tqdm(range(args.len_event)):
        # check event team home or away 
        if events['Team'].iloc[event_id] == 'Home':
            _, _, _, attacking_players = mpc.generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, params)
